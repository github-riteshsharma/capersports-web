...
// @route   GET /api/products/categories/with-products
// @desc    Get categories with sample products - optimized single call
// @access  Public
router.get('/categories/with-products', async (req, res) => {
  try {
    const distinctCategories = await Product.distinct('category', { isActive: true });
    const validCategories = distinctCategories.filter(cat => cat && cat.trim() !== '').slice(0, 4);
    ...
    categoriesWithData.sort((a, b) => a._id.localeCompare(b._id));
